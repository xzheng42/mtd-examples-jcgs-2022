################################################################################
### Description: Make plots of the weights of the GMTD model ran in the first
...
load(paste0(path1, 'res_sim1.RData'))
...
png(paste0(path2, "15_DIR", ".png"), units = "in", width = 6, height = 5, res = 300)
plotWeight(res_L15[[1]], mtdorder, DIR_prior_mu, true_weight, "",  "Probability")
dev.off()

png(paste0(path2, "15_SB", ".png"), units = "in", width = 6, height = 5, res = 300)
plotWeight(res_L15[[2]], mtdorder, SB_prior_mu, true_weight)
dev.off()

png(paste0(path2, "15_CDP", ".png"), units = "in", width = 6, height = 5, res = 300)
plotWeight(res_L15[[3]], mtdorder, CDP_prior_mu, true_weight)
dev.off()

################################################################################
### Figures 1-2 of Supplementary Material
################################################################################
# order L = 5 ------------------------------------------------------------------
mtdorder <- 5
alpha <- 1
