################################################################################
### Description: This code runs the second scenario of the simulation study 
###              presented in the paper. 
### Output: A data set called "res_sim2.RData" that contains posterior samples 
###         for the GMTD model in each case of the second scenario, running time 
###         for each case, and true weight.
################################################################################
rm(list = ls())
...
set.seed(42)
J <- 5
true_weight <- c(0.2, 0.05, 0.45, 0.05, 0.25)
param <- list(rho = c(0.4, 0.1, 0.7, 0.1, 0.5), mu = 10, sigma2 = 100)
init_data <- c(8, 12, 5, 9, 7)
ndata <- 2000
...
save(res_L5, res_L15, res_L25, runtime_L5, runtime_L15, runtime_L25, true_weight, 
     file = paste0(path, 'res_sim2.RData'))
